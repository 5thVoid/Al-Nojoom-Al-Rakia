...
describe("OrderService", () => {
  ...
  describe("checkout", () => {
    it("creates order and clears cart", async () => {
      ...
      const order = await orderService.checkout(1);

      expect(Order.create).toHaveBeenCalledWith(
        expect.objectContaining({ subtotal: 100, total: 100, userId: 1 }),
        expect.any(Object)
      );
      ...
      expect(order).toMatchObject({ id: 100, total: 100 });
    ...
    it("throws when cart is empty", async () => {
      ...
      await expect(orderService.checkout(1)).rejects.toThrow("Cart is empty");
    ...
    it("throws when inventory insufficient", async () => {
      jest.spyOn(Cart, "findOrCreate").mockResolvedValue([{ id: 1 } as any, false]);
      jest.spyOn(CartItem, "findAll").mockResolvedValue([
        { productId: 10, quantity: 3, Product: { price: 50 } },
      ] as any);
      jest
        .spyOn(Inventory, "findAll")
